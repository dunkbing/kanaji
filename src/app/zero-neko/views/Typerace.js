    ...
    return(
        <div className="flex flex-col mx-4 lg:mx-96 py-4 lg:p-8 space-y-6 lg:space-y-8">
            ...
            <div className="flex flex-col lg:flex-row mx-auto space-y-6 lg:space-y-0 lg:space-x-2">
                <input onChange={(e) => handleChange(e)} ref={node => (input = node)} autoComplete="off" disabled={result === true ? true : false} className="mx-auto transition-all duration-150 border-2 border-gray-200 focus:border-primary px-5 py-2 rounded-lg focus:outline-none text-center" type="text" name="inputWord" id="inputWord" placeholder={result ? "Please Reset" :"Type the answer"}/>
                <button className=" mx-auto bg-primary hover:bg-opacity-70 bg-opacity-80 text-white focus:outline-none active:ring-2 active:ring-primary px-5 py-3 rounded-lg ring-offset-2" onClick={() => handlePass()}>Pass</button>
            </div>
            <div className="flex mx-auto space-x-2">
                <button className="mx-auto bg-gray-200 active:ring-2 active:ring-primary ring-offset-2 bg-opacity-80 hover:bg-opacity-70 focus:outline-none px-5 py-3 rounded-lg cursor-pointer" onClick={() => handleReset()}>Reset</button>
                <label  htmlFor="useKanji" className={(useKanji ? "bg-blue-400 text-white" : "bg-gray-200")+" flex mx-auto hover:bg-opacity-80 px-5 py-3 rounded-lg cursor-pointer focus-within:ring-2 focus-within:ring-primary"}>
                    <span>Kanji</span>
